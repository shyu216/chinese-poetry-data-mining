<!--
  ...
  数据管线:
    ...
    - 搜索: `useWordSearch` 提供快速检索，按需加载词表分片。
...
import {
  TextOutline,
  LibraryOutline, StarOutline, ResizeOutline, FlameOutline
...
import { getMetadata, getCache, getVerifiedChunkedCache } from '@/composables/useCache'
...
// 合并后的统计计算属性
const globalTotalWords = computed(() => wordcount.totalWords.value || 0)
const globalTotalChunks = computed(() => wordcount.totalChunks.value || 0)
...
const loadData = async () => {
  ...
  try {
    ...
    const runWordcountNetworkLoad = () =>
      chunkLoader.loadChunks<WordCountItem[]>(unloadedChunkIds, wordcount.loadChunk, {
        concurrency: 5,
        chunkDelay: 0,
...
<template>
  <div class="wordcount-view">
    ...
    <NGrid :cols="4" :x-gap="16" :y-gap="16" class="stats-grid">
      ...
      <NGridItem>
        <StatsCard label="最高频词" :value="topWord" :prefix-icon="FlameOutline" />
      </NGridItem>
      <NGridItem>
